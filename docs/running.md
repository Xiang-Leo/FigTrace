...
自动化测试覆盖分片续传、服务重启、内容校验、路径限制、同名文件、预览失败隔离、PDF/TIFF 分页、目录迁移、版本与源文件关联、批量元数据更新和备份恢复。CI 已配置 Windows、macOS、Linux 矩阵，但配置本身不代表这些平台已通过远端执行。

本轮本机测量（macOS、Python 3.14）：5,000 个 32×24 PNG 文件索引耗时约 8.49 秒；30 次预热 API 查询每页 48 条，中位数 10.5 毫秒，P95 11.7 毫秒。该测试不含 5,000 张缩略图转换、浏览器渲染、真实大文件或网盘读取，不作为通用性能保证。
